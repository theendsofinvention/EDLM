...
def get_template(ctx: Context):
    ...
    while not ctx.template_source:
        ...
        if len(this_folder.parents) is 1:
            ctx.warning('no template found, using default "template.tex"')
            default_template = Path(ctx.source_folder, 'template.tex')
            default_template.write_text(DEFAULT_TEMPLATE, encoding='utf8')
            ctx.template_file = default_template
            break
