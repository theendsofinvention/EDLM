...
_INFO_BOX_RE = re.compile(r'/info\((?P<text>.*?)\)', flags=re.MULTILINE + re.DOTALL)
_INFO_BOX_REPLACE = r'\\begin{infobox}\n%s\n\end{infobox}'

_ERROR_BOX_RE = re.compile(r'/error\((?P<text>.*?)\)', flags=re.MULTILINE + re.DOTALL)
...
def process_boxes(ctx: Context) -> None:
    """
    Processes boxes in the markdown text
